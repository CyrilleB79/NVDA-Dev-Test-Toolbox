		...
		while nReadLines < 3:
			...
			if RE_STACK_TRACE_LINE.search(ti.text.strip()):
				...
				if includeContext:
					...
					selEnd = tbFrame.errContextLocation[1] - tbFrame.errContextLocation[0]
				else:
					selStart = tbFrame.errLocation[0]
					selEnd = tbFrame.errLocation[1] - tbFrame.errLocation[0]
				ti.move(textInfos.UNIT_CHARACTER, selStart)
				ti.move(textInfos.UNIT_CHARACTER, selEnd, "end")
				if select:
					ti.updateSelection()
				else:
					text = ti.text

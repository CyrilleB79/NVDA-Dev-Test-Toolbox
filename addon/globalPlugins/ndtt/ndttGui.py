...
class NDTTSettingsPanel(gui.settingsDialogs.SettingsPanel):
	...
	def makeSettings(self, sizer):
		...
		self.copyReverseTranslation.SetValue(
			config.conf["ndtt"]["copyRevTranslation"] and not globalVars.appArgs.secure,
		)
	...
	def onSave(self):
		...
		if not (config.conf.profiles[-1].name is None and len(config.conf.profiles) == 1):
			log.debugWarning(
				"No configuration saved for NDTT since the current profile is not the default one.",
			)
			return
		...
		if not config.conf["ndtt"]["preserveConsoleInputHistory"]:
			plugin = [p for p in globalPluginHandler.runningPlugins if p.__module__ == "globalPlugins.ndtt"][
				0
			]
		...
		config.conf["ndtt"]["functionCallsLogMethod"] = logMethodDisplayStrings[
			self.functionCallsLogMethodsList.Selection
		][0]
